kamaMultiSelect.$inject = ['$q', 'alertService', 'toolsService', '$timeout'];
export default function kamaMultiSelect($q, alertService, toolsService, $timeout) {
    var directive = {
        link: link
        , template: `<select class="form-control kama-select" style="width: 100%"
                ng-options="item[obj.displayName] for item in obj.items track by item.ID"
                ng-model="selected"
        ...
        function update() {
                ...
                && scope.obj.bindingObject.model[scope.obj.model].length)
            {
                for (let i = 0; i < scope.obj.items.length; i++) {
                    ...
                    for (let j = 0; j < scope.obj.bindingObject.model[scope.obj.model].length; j++) {
            ...
            $timeout(() => {
                selectElement
                    .val(selectedValues.map((val) => { return val[scope.obj.uniqueId] }))
                    .trigger('change');
            })
        }
